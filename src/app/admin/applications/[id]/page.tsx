...
import { ArrowLeft, Sparkles, User, Mail, Phone, MapPin, Briefcase, FileText } from "lucide-react";
...
export default function ApplicationReviewPage({ params }: { params: { id: string } }) {
  ...
  const { data: application, isLoading } = useDoc(doc(firestore, "applications", params.id));
  ...
  const handleSummarize = async () => {
    ...
    try {
      // For demonstration, we use a placeholder resume URI if one isn't provided
      // In a production app, you would fetch the actual base64 data of the resume PDF
  ...
  return (
    <EmployeeLayout isAdmin>
      <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
        <Link href="/admin" className="inline-flex items-center gap-2 text-primary font-bold hover:text-accent transition-colors">
